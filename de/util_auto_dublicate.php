...
$dates = array();
$newdate ="";
if(isset($_POST['todo_ids'])){
    $todo_ids=$_POST['todo_ids'];
}
if(isset($_POST['dates'])){
    $dates=$_POST['dates'];
}

$len_todo_ids = sizeof($todo_ids);
$len_dates = sizeof($dates);
...
for ($xy = 0; $xy < $len_todo_ids; $xy++) {
    $newdate = $dates[$xy];
    $id = $todo_ids[$xy];
    $newdate2 = $newdate;
    $date_update_q="UPDATE `tbl_todo_notification` SET `duplicate`= 1 WHERE `tdl_id` =  '$id' AND `date` = '$newdate2'";
    $result_date_update = $conn->query($date_update_q);
}
...
for ($xy = 0; $xy < $len_todo_ids; $xy++) {
    ...
    $entry_time =  date_format($newdate,"Y-m-d H:i:s");
    $last_edit_time =  date_format($newdate,"Y-m-d H:i:s");
    $current_date =   date("Y-m-d");
    if($result_date_update){
        $get_name_todo = "";
        $create_as = "";
        $select_one = "";
        ...
        $sql="INSERT INTO `tbl_todolist` (`title`, `title_it`, `title_de`, `description`, `description_it`, `description_de`, `visibility_status`, `assign_recipient_status`, `due_date`, `repeat_status`, `day`, `repeat_until`, `no_of_repeat`, `tags`, `hotel_id`, `saved_status`, `status_id`, `is_active`, `is_delete`,`entrybyid`,`entrybyip`,`editbyid`,`editbyip`) SELECT `title`,`title_it`,`title_de`,`description`,`description_it`,`description_de`,`visibility_status`,`assign_recipient_status`,`due_date`,`repeat_status`,`day`,`repeat_until`,`no_of_repeat`,`tags`,`hotel_id`,`saved_status`,`status_id`,`is_active`,`is_delete`,`entrybyid`,`entrybyip`,`editbyid`,`editbyip` FROM`tbl_todolist`  WHERE `tdl_id` = $id";
        $result = $conn->query($sql);
        if($result){
            $last_id = $conn->insert_id;
            echo $last_id;

            $sql_update_satus="UPDATE `tbl_todolist` SET `status_id` = 9 WHERE `tdl_id` = $id";
            $result_status = $conn->query($sql_update_satus);

            $sql_id_update="UPDATE `tbl_todo_notification` SET `tdl_id`=$last_id WHERE `tdl_id` = $id";
            $result_id_update = $conn->query($sql_id_update);


            if($current_date > $check_date){

                $sql ="UPDATE `tbl_todolist` SET `entrytime`='$entry_time',`edittime`='$last_edit_time',status_id  = 9  WHERE `tdl_id` =$last_id";
                $result = $conn->query($sql);
            }else{
                $sql ="UPDATE `tbl_todolist` SET `entrytime`='$entry_time',`edittime`='$last_edit_time',status_id  = 6  WHERE `tdl_id` =$last_id";
                $result = $conn->query($sql);
            }
            if($result){
                //Recipent
