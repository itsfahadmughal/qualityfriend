    ...
    if($result){
        ...
        for ($x = 0; $x < $lenth; $x++) {
            ...
            if($save_as == 'CREATE' && $visibility_status != "PRIVATE"){
                ...
                if ($result_user && $result_user->num_rows > 0) {
                    while($row_user = mysqli_fetch_array($result_user)) {
                        $sql_alert="INSERT INTO `tbl_alert`(`user_id`, `id`, `id_name`, `id_table_name`, `alert_message`, `alert_type`, `hotel_id`, `entrytime`) VALUES ('$row_user[0]','$hdb_id','hdb_id','tbl_handbook','$title Update','UPDATE','$hotel_id','$entry_time')";
                        $result_alert = $conn->query($sql_alert);
