...
$fields = array('NEIN', 'MITARBEITERNAME','ABTEILUNG', 'TITEL DER VERSCHIEBUNG', 'DATUM', 'STARTZEIT', 'ENDZEIT', 'GESAMTSTUNDEN', 'BRECHEN (MIN)', 'LOHN PRO STUNDE','LOHNART', 'Gesamtlohn'); 
...
if($export_type == "CSV"){
    ...
    fputcsv($f, array("Bericht über die geleistete Arbeitszeit"), $delimiter); 
    ...
    if ($result_employee && $result_employee->num_rows > 0) {
        while($row_employee = mysqli_fetch_array($result_employee)) {
            ...
            if($nk == 0){
                array_push($days,"Gesamt");
                $lineData = array_merge(array("Nr.","Name",""),$days);
            ...
            $lineData = array_merge(array(($nk+1),$employe_name,"Geleistete Stunden"),$worked_hours);
            fputcsv($f, $lineData, $delimiter); 
            $lineData = array_merge(array("Abteilung",$row_employee['department_name'],"Bezahlter Urlaub"),$paid_holiday);
            fputcsv($f, $lineData, $delimiter); 
            $lineData = array_merge(array("Löhne Typ",$row_employee['wage_type'],"Unbezahlter Urlaub"),$unpaid_holiday);
            fputcsv($f, $lineData, $delimiter); 
            $lineData = array_merge(array("Löhne:",$row_employee['wage'],"Krank"),$sick);
            fputcsv($f, $lineData, $delimiter); 
