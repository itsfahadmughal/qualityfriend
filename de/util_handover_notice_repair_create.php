        ...
        if($save_as == "TEMPLATE"){
            ...
            if($result && $result_d2 && $result_d){
                ...
                for ($x = 0; $x < sizeof($recipents_); $x++) {
                    ...
                    $sql1="INSERT INTO `tbl_handover_recipents`(`hdo_id`,`user_id`) VALUES ($id,'$recipents_[$x]')";
                    $result1 = $conn->query($sql1);
                    if($save_as == 'CREATE' && $visibility_status != "PRIVATE"){
                            ...
                            while($row_user = mysqli_fetch_array($result_user)) {
                                if(!in_array($row_user['user_id'],$recipents_)){
                                    ...
                                    if($user_token != ""){
                                        ...
                                        $send_notification_array =  array();
                                        $nested_array =  array();
                                        $nested_array['to'] = $user_token;
                                        $nested_array['title'] = $title_send;
                                        $nested_array['body'] = $body_send;
                                        array_push($send_notification_array, $nested_array);
                                        $result = sendExpoMsg($send_notification_array);
                                    }
                                }
                            }
                        }
