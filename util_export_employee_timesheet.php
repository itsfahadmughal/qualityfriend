...
$fields = array('No', 'EMPLOYEE NAME','DEPARTMENT', 'SHIFT TITLE', 'DATE', 'START TIME', 'END TIME', 'TOTAL HOURS', 'BREAK (MIN)', 'SHIFT COMPLETED', 'WAGE PER HOUR','WAGE TYPE', 'TOTAL WAGE'); 
...
if($export_type == "CSV"){
    ...
    while ($row = mysqli_fetch_array($res)) {
        ...
        $lineData[] = array('No' => $n,'EMPLOYEE NAME' => $row['firstname'].' '.$row['lastname'],'DEPARTMENT' => $row['department_name'],'SHIFT TITLE' => $row['title'],'DATE' => $row['date'],'START TIME' => $row['start_time'],'END TIME' => $row['end_time'],'TOTAL HOURS' => $row['total_hours'],'BREAK (MIN)' => $row['shift_break'], 'SHIFT COMPLETED' => $completed,'WAGE PER HOUR' => $row['wage'],'WAGE TYPE' => $row['wage_type'],'TOTAL WAGE' => $row['wage']*$row['total_hours']);
