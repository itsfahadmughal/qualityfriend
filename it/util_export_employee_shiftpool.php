...
$fields = array('No', 'OFFERTA DI','DATA OFFERTA','OFFRIRE A', 'TURNO OFFERTO', 'DATA DEL TURNO', 'ORARIO INIZIO TURNO', 'ORA FINE TURNO', 'APPROVATO DAL DIPENDENTE', 'APPROVATO DALL`ADMIN'); 

$fields2 = array('No', 'OFFERTA DI','DATA OFFERTA', 'TURNO OFFERTO', 'DATA OFFERTA DEL TURNO', 'TURNO OFFERTO ORA DI INIZIO', 'ORARIO DI FINE DEL TURNO OFFERTO', 'OFFRIRE A', 'CAMBIO TURNO CON', 'DATA DEL TURNO', 'ORARIO INIZIO TURNO', 'ORA FINE TURNO', 'APPROVATO DAL DIPENDENTE', 'APPROVATO DALL`ADMIN'); 
$empty = array('COMMERCIO DI SPOSTAMENTO SOTTO', ' ',' ', ' ', ' ', ' ', ' ', ' ', ' ', ' ', ' ', ' ', ' ', ' '); 
...
if($export_type == "CSV"){
    ...
    $res = $conn->query($sql);
    $lineData[] = array("TURNI OFFERTI" => 'SÌ');
    $n=1;
    while ($row = mysqli_fetch_array($res)) {
        if($row['is_approved_by_employee'] == "1"){
            $row['is_approved_by_employee'] = "SÌ";
        ...
        $lineData[] = array('No' => $n,'OFFERTA DI' => $row['offer_by'],'DATA OFFERTA' => $row['entrytime'],'OFFRIRE A' => $row['offer_to'],'TITOLO' => $row['title'],'DATE' => $row['date'],'ORA DI INIZIO' => $row['start_time'],'TEMPO SCADUTO' => $row['end_time'],'APPROVATO DAL DIPENDENTE' => $row['is_approved_by_employee'], 'APPROVATO DALL`ADMIN' => $row['is_approved_by_admin']);

        $n++;
    }
    $lineData[] = array("TURNI COMMERCIALI" => 'SÌ');
    $res2 = $conn->query($sql_t);
    $n=1;
    while ($row2 = mysqli_fetch_array($res2)) {
        if($row2['is_approved_by_employee'] == "1"){
        ...
        $lineData[] = array('No' => $n,'OFFERTA DI' => $row2['offer_by'],'DATA OFFERTA' => $row2['entrytime'],'TURNO OFFERTO' => $row2['title'],'DATA OFFERTA DEL TURNO' => $row2['date'],'TURNO OFFERTO ORA DI INIZIO' => $row2['start_time'],'ORARIO DI FINE DEL TURNO OFFERTO' => $row2['end_time'],'OFFRIRE A' => $row2['offer_to'],'CAMBIO TURNO CON' => $row2['t'],'DATA DEL TURNO' => $row2['dt'],'ORARIO INIZIO TURNO' => $row2['st'],'ORA FINE TURNO' => $row2['et'],'APPROVATO DAL DIPENDENTE' => $row2['is_approved_by_employee'], 'APPROVATO DALL`ADMIN' => $row2['is_approved_by_admin']);
    ...
    <table id="demo-foo-addrow" class="table table-bordered m-t-30 table-hover contact-list table-striped" data-paging="true" data-paging-size="25">
        <thead>
            <tr>
                ...
                <th class="">DATA OFFERTA</th>
                <th class="">OFFRIRE A</th>
                <th class="">TITOLO</th>
                <th class="">DATA DEL TURNO</th>
                <th class="">ORARIO INIZIO TURNO</th>
                <th class="">ORA FINE TURNO</th>
                <th class="">APPROVATO DAL DIPENDENTE</th>
                <th class="">APPROVATO DALL'ADMIN</th>
            </tr>
        </thead>
        <tbody>
            <?php
    ...
    <table id="demo-foo-addrow" class="table table-bordered m-t-30 table-hover contact-list table-striped" data-paging="true" data-paging-size="25">
        <thead>
            <tr>
                ...
                <th class="">DATA OFFERTA DEL TURNO</th>
                <th class="">TURNO OFFERTO ORA DI INIZIO</th>
                <th class="">ORARIO DI FINE DEL TURNO OFFERTO</th>
                <th class="">OFFRIRE A</th>
                <th class="">CAMBIO TURNO CON</th>
                <th class="">DATA DEL TURNO</th>
                <th class="">ORARIO INIZIO TURNO</th>
                <th class="">ORA FINE TURNO</th>
                <th class="">APPROVATO DAL DIPENDENTE</th>
