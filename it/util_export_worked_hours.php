...
$fields = array('No', 'NOME DIPENDENTE','DIPARTIMENTO', 'TITOLO DEL CAMBIO', 'DATE', 'ORA DI INIZIO', 'TEMPO SCADUTO', 'ORE TOTALI','ROTTURA (MIN)', 'SALARIO ORARIO','TIPO DI SALARIO', 'SALARIO TOTALE'); 
...
if($export_type == "CSV"){
    ...
    fputcsv($f, array("Rapporto sull'orario di lavoro"), $delimiter); 
    ...
    if ($result_employee && $result_employee->num_rows > 0) {
        while($row_employee = mysqli_fetch_array($result_employee)) {
            ...
            if($nk == 0){
                array_push($days,"Totale");
                $lineData = array_merge(array("Nr.","Nome",""),$days);
            ...
            $lineData = array_merge(array(($nk+1),$employe_name,"Ore lavorate"),$worked_hours);
            fputcsv($f, $lineData, $delimiter); 
            $lineData = array_merge(array("Dipartimento",$row_employee['department_name'],"Vacanze pagate"),$paid_holiday);
            fputcsv($f, $lineData, $delimiter); 
            $lineData = array_merge(array("Salari Tipo",$row_employee['wage_type'],"Ferie non pagate"),$unpaid_holiday);
            fputcsv($f, $lineData, $delimiter); 
            $lineData = array_merge(array("Salari:",$row_employee['wage'],"Malato"),$sick);
            fputcsv($f, $lineData, $delimiter); 
    ...
    while ($row = mysqli_fetch_array($res)) {
        ...
        $lineData[] = array('No' => $n,'NOME DIPENDENTE' => $row['firstname'].' '.$row['lastname'],'DIPARTIMENTO' => $row['department_name'],'TITOLO DEL CAMBIO' => $row['title'],'DATE' => $row['date'],'ORA DI INIZIO' => $row['start_time'],'TEMPO SCADUTO' => $row['end_time'],'ORE TOTALI' => $row['total_hours'],'ROTTURA (MIN)' => $row['shift_break'],'SALARIO ORARIO' => $row['wage'],'TIPO DI SALARIO' => $row['wage_type'],'SALARIO TOTALE' => $row['wage']*$row['total_hours']);
