...
$fields = array('No', 'RICHIESTA DA', 'CATEGORIA', 'DURATA', 'DATE', 'INIZIO', 'FINE', 'ORE TOTALI','ROTTURA (MIN)', 'STATO', 'APPROVATO/RIFIUTATO DA','TEMPO DI APPROVAZIONE/RIFIUTO'); 
...
if($export_type == "CSV"){
    ...
    if ($result && $result->num_rows > 0) {
        while($row = mysqli_fetch_array($result)) {
            ...
            if($prev_id != $row['request_by']){
                $lineData = array("","", "", "", "", "","", "Somma totale", $grand_total_hours,"", "", "","",);
    ...
    while ($row = mysqli_fetch_array($res)) {
        ...
        $lineData[] = array('No' => $n,'RICHIESTA DA' => $row['fullname'],'CATEGORIA' => $row['category'],'DURATA' => $row['duration'],'DATE' => $row['date'],'INIZIO' => $row['start_time'],'FINE' => $row['end_time'],'ORE TOTALI' => $row['total_hours'],'ROTTURA (MIN)' => $row['shift_break'], 'STATO' => $row['status'],'APPROVATO/RIFIUTATO DA' => $row['fullname_approved'],'TEMPO DI APPROVAZIONE/RIFIUTO' => $row['edittime']);
    ...
    <table id="demo-foo-addrow" class="table table-bordered m-t-30 table-hover contact-list table-striped" data-paging="true" data-paging-size="25">
        <thead>
            <tr>
                ...
                <th class="">STATO</th>
                <th class="">APPROVATO/RIFIUTATO DA</th>
                <th class="">TEMPO DI APPROVAZIONE/RIFIUTO</th>
