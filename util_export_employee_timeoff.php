...
$fields = array('No', 'REQUEST BY', 'CATEGORY', 'DURATION', 'DATE', 'START', 'END', 'TOTAL HOURS','BREAK (MIN)', 'STATUS', 'APPROVED/DECLINE BY','APPROVED/DECLINE TIME'); 
...
if($export_type == "CSV"){
    ...
    while ($row = mysqli_fetch_array($res)) {
        ...
        $lineData[] = array('No' => $n,'REQUEST BY' => $row['fullname'],'CATEGORY' => $row['category'],'DURATION' => $row['duration'],'DATE' => $row['date'],'START' => $row['start_time'],'END' => $row['end_time'],'TOTAL HOURS' => $row['total_hours'],'BREAK (MIN)' => $row['shift_break'], 'STATUS' => $row['status'],'APPROVED/DECLINE BY' => $row['fullname_approved'],'APPROVED/DECLINE TIME' => $row['edittime']);
    ...
    <table id="demo-foo-addrow" class="table table-bordered m-t-30 table-hover contact-list table-striped" data-paging="true" data-paging-size="25">
        <thead>
            <tr>
                ...
                <th class="">STATUS</th>
                <th class="">APPROVED/DECLINE BY</th>
                <th class="">APPROVED/DECLINE TIME</th>
