...
if(file_exists("util_config.php") && is_readable("util_config.php") && include("util_config.php")) 
{
    ...
    if($user_id == 0 || $hotel_id == 0){
        ...
        if($name == "complate_admin"){
            $sql="UPDATE `tbl_todolist` SET `status_id`='$status_id' WHERE `tdl_id` = '$todo_id'";
            $title ="";
            $created_by = 0;
            $sql_sub33="SELECT * FROM `tbl_todolist` WHERE `tdl_id` = $todo_id";
            ...
            if ($result_get_recipent && $result_get_recipent->num_rows > 0) {
                while($row_get_recipent = mysqli_fetch_array($result_get_recipent)) {
                    ...
                    $priority = 0; 
                    //getpoerity
                    $sql_get_recipent_p="SELECT * FROM `tbl_todolist_recipents` WHERE `tdl_id` =  $todo_id and user_id = $user_id";
                    $result_get_recipent_p = $conn->query($sql_get_recipent_p);
                    if ($result_get_recipent_p && $result_get_recipent_p->num_rows > 0) {
                        while($row_get_recipent_p = mysqli_fetch_array($result_get_recipent_p)) {
                            $priority = $row_get_recipent_p['priority'];
                    ...
                    $sql_alert="INSERT INTO `tbl_alert`(`user_id`, `id`, `id_name`, `id_table_name`, `alert_message`, `alert_type`, `hotel_id`, `entrytime`,`priority`) VALUES ('$user_id','$todo_id','tdl_id','tbl_todolist','$aleart_msg','UPDATE','$hotel_id','$entry_time',$priority)";
                    $result_alert = $conn->query($sql_alert);
                    $check_user = $user_id;
                    //getUserToken
                    $user_token  = "";
