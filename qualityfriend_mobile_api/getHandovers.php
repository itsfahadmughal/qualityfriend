    ...
    if($hotel_id != 0){
        if($handover_id == 0){
            if($Create_edit_handovers == 1 || $usert_id == 1){
                $sql="SELECT * FROM `tbl_handover` WHERE `is_delete` = 0 AND `hotel_id` = $hotel_id AND status_id = 6 AND saved_status = '$saved_status' ORDER BY 1 DESC";
            } else{
            ...
            $sql="SELECT * FROM `tbl_handover` WHERE  `is_delete` = 0 AND `hotel_id` = $hotel_id AND hdo_id = $handover_id ORDER BY 1 DESC";
        }
        ...
        if ($result && $result->num_rows > 0) {
            while($row = mysqli_fetch_array($result)) {
                ...
                if($is_completed == 0 || $handover_id != 0){
                    ...
                    //Hide one
                    if ($handover_id != 0 || $saved_status == 'TEMPLATE' ){
                    ...
                    $sql2="SELECT * FROM `tbl_handover_attachment` WHERE hdo_id = $hdo_id";
                    $result2 = $conn->query($sql2);
                    if ($result2 && $result2->num_rows > 0) {
                        while($row2 = mysqli_fetch_array($result2)) {
                            $temp3['attachment_id'] = $row2['hdoa_id'];
