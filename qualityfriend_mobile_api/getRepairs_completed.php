    ...
    if($hotel_id != 0){
        if($repair_id == 0){
            if($Create_edit_repairs == 1 || $usert_id == 1){
                $sql="SELECT * FROM `tbl_repair` WHERE `hotel_id` = $hotel_id AND status_id = 8 and is_delete = 0 and saved_status = '$saved_status' ORDER BY 1 DESC";
            }else{
            ...
            $sql="SELECT * FROM `tbl_repair`  AND `is_delete` = 0 AND `hotel_id` = $hotel_id AND rpr_id = $repair_id ORDER BY 1 DESC";
        }
