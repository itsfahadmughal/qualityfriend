...
if(file_exists("../util_config.php") && is_readable("../util_config.php") && include("../util_config.php")) 
{
    ...
    $sql_inner_1_last="SELECT SUM(CASE WHEN MONTH(`arrival`) != MONTH(`departure`) THEN ((adults+infants+children) * (DATEDIFF( LAST_DAY(`arrival`), `arrival`) + 1)) ELSE (adults+infants+children) * (DATEDIFF(`departure`, `arrival`)) END) as stay_off, SUM(CASE WHEN MONTH(`arrival`) != MONTH(`departure`) THEN (`accommodation_sale` / (DATEDIFF(`departure`, `arrival`))) * (DATEDIFF( LAST_DAY(`arrival`), `arrival`) + 1) + (`additionalServices_sale` / (DATEDIFF(`departure`, `arrival`))) * (DATEDIFF( LAST_DAY(`arrival`), `arrival`) + 1) ELSE `accommodation_sale`+`additionalServices_sale` END) as acc_sale FROM `tbl_forecast_reservations_rooms` WHERE hotel_id = $hotel_id AND MONTH(`arrival`) = $month_current AND YEAR(`arrival`) = $year_last AND `status` IN ('reserved','roomFixed','departed','occupied')";
    $result_inner_1_last = $conn->query($sql_inner_1_last);

    $sql_inner_11_last="SELECT SUM(CASE WHEN MONTH(`arrival`) != MONTH(`departure`) THEN ((adults+infants+children) * (DATEDIFF( `departure`,DATE_SUB(`departure`,INTERVAL DAYOFMONTH(`departure`)-1 DAY)) + 1)) ELSE 0 END) as stay_off , SUM(CASE WHEN MONTH(`arrival`) != MONTH(`departure`) THEN (`accommodation_sale` / (DATEDIFF(`departure`, `arrival`))) * (DATEDIFF( `departure`,DATE_SUB(`departure`,INTERVAL DAYOFMONTH(`departure`)-1 DAY))) + (`additionalServices_sale` / (DATEDIFF(`departure`, `arrival`))) * (DATEDIFF( `departure`,DATE_SUB(`departure`,INTERVAL DAYOFMONTH(`departure`)-1 DAY)) + 1) ELSE 0 END) as acc_sale FROM `tbl_forecast_reservations_rooms` WHERE hotel_id = $hotel_id AND MONTH(`departure`) = $month_current AND YEAR(`departure`) = $year_last AND `status` IN ('reserved','roomFixed','departed','occupied')";
    ...
    $sql_inner_last_day="SELECT SUM(CASE WHEN MONTH(`arrival`) != MONTH(`departure`) THEN ((adults+infants+children) * (DATEDIFF( LAST_DAY(`arrival`), `arrival`))) ELSE (adults+infants+children) * (DATEDIFF(`departure`, `arrival`)) END) as stay_off, SUM(CASE WHEN MONTH(`arrival`) != MONTH(`departure`) THEN (`accommodation_sale` / (DATEDIFF(`departure`, `arrival`))) * (DATEDIFF( LAST_DAY(`arrival`), `arrival`)) + (`additionalServices_sale` / (DATEDIFF(`departure`, `arrival`))) * (DATEDIFF( LAST_DAY(`arrival`), `arrival`)) ELSE `accommodation_sale`+`additionalServices_sale` END) as acc_sale FROM `tbl_forecast_reservations_rooms` WHERE hotel_id = $hotel_id AND `date` = '$last_day' AND `status` IN ('reserved','roomFixed','departed','occupied')";
    $result_inner_last_day = $conn->query($sql_inner_last_day);

    $sql_inner_last_day1="SELECT SUM(CASE WHEN MONTH(`arrival`) != MONTH(`departure`) THEN ((adults+infants+children) * (DATEDIFF( `departure`,DATE_SUB(`departure`,INTERVAL DAYOFMONTH(`departure`)-1 DAY)))) ELSE 0 END) as stay_off , SUM(CASE WHEN MONTH(`arrival`) != MONTH(`departure`) THEN (`accommodation_sale` / (DATEDIFF(`departure`, `arrival`))) * (DATEDIFF( `departure`,DATE_SUB(`departure`,INTERVAL DAYOFMONTH(`departure`)-1 DAY))) + (`additionalServices_sale` / (DATEDIFF(`departure`, `arrival`))) * (DATEDIFF( `departure`,DATE_SUB(`departure`,INTERVAL DAYOFMONTH(`departure`)-1 DAY))) ELSE 0 END) as acc_sale FROM `tbl_forecast_reservations_rooms` WHERE hotel_id = $hotel_id AND `date` = '$last_day' AND `status` IN ('reserved','roomFixed','departed','occupied')";
    ...
    $sql_inner_last_day_last="SELECT SUM(CASE WHEN MONTH(`arrival`) != MONTH(`departure`) THEN ((adults+infants+children) * (DATEDIFF( LAST_DAY(`arrival`), `arrival`))) ELSE (adults+infants+children) * (DATEDIFF(`departure`, `arrival`)) END) as stay_off, SUM(CASE WHEN MONTH(`arrival`) != MONTH(`departure`) THEN (`accommodation_sale` / (DATEDIFF(`departure`, `arrival`))) * (DATEDIFF( LAST_DAY(`arrival`), `arrival`)) + (`additionalServices_sale` / (DATEDIFF(`departure`, `arrival`))) * (DATEDIFF( LAST_DAY(`arrival`), `arrival`)) ELSE `accommodation_sale`+`additionalServices_sale` END) as acc_sale FROM `tbl_forecast_reservations_rooms` WHERE hotel_id = $hotel_id AND `date` = '$last_day_last' AND `status` IN ('reserved','roomFixed','departed','occupied')";
    $result_inner_last_day_last = $conn->query($sql_inner_last_day_last);

    $sql_inner_last_day1_last="SELECT SUM(CASE WHEN MONTH(`arrival`) != MONTH(`departure`) THEN ((adults+infants+children) * (DATEDIFF( `departure`,DATE_SUB(`departure`,INTERVAL DAYOFMONTH(`departure`)-1 DAY)))) ELSE 0 END) as stay_off , SUM(CASE WHEN MONTH(`arrival`) != MONTH(`departure`) THEN (`accommodation_sale` / (DATEDIFF(`departure`, `arrival`))) * (DATEDIFF( `departure`,DATE_SUB(`departure`,INTERVAL DAYOFMONTH(`departure`)-1 DAY))) + (`additionalServices_sale` / (DATEDIFF(`departure`, `arrival`))) * (DATEDIFF( `departure`,DATE_SUB(`departure`,INTERVAL DAYOFMONTH(`departure`)-1 DAY))) ELSE 0 END) as acc_sale FROM `tbl_forecast_reservations_rooms` WHERE hotel_id = $hotel_id AND `date` = '$last_day_last' AND `status` IN ('reserved','roomFixed','departed','occupied')";
