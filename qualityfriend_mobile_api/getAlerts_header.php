...
if(file_exists("util_config.php") && is_readable("util_config.php") && include("util_config.php")) 
{
    ...
    if ($result_not && $result_not->num_rows > 0) {
        while($row_not = mysqli_fetch_array($result_not)) {
            ...
            if($row_not['id_table_name'] == "tbl_handover"){ $temp['alert_type'] = 'Handover';}else if($row_not['id_table_name'] == "tbl_repair"){ $temp['alert_type'] = 'Repair';}else if($row_not['id_table_name'] == "tbl_note"){ $temp['alert_type'] = 'notice';}else if($row_not['id_table_name'] == "tbl_handbook"){ $temp['alert_type'] = 'Handbook'; }else if($row_not['id_table_name'] == "tbl_todolist"){ $temp['alert_type'] = 'Todolist'; }else if($row_not['id_table_name'] == "tbl_applicants_employee"){ $temp['alert_type'] = 'Recruiting'; }else if($row_not['id_table_name'] == "tbl_create_job"){$temp['alert_type'] = 'Recruiting'; }else if($row['id_table_name'] == "tbl_shifts"){$temp['alert_type'] = 'Schedules'; }  else if($row['id_table_name'] == "tbl_shift_events"){$temp['alert_type'] = 'Events'; }else if($row['id_table_name'] == "tbl_shift_offer"){$temp['alert_type'] = 'Shift Offered'; } else if($row['id_table_name'] == "tbl_shift_trade"){$temp['alert_type'] = 'Shift Trade'; } else if($row['id_table_name'] == "tbl_time_off"){$temp['alert_type'] = 'Off Time'; }
