...
if(file_exists("util_config.php") && is_readable("util_config.php") && include("util_config.php")) 
{
    ...
    $hotel_id= $usert_id= $depart_id= $Create_edit_repairs= $repair_id=0;
    $saved_status="";
    ...
    if(isset($_POST["create_edit_rule"])){
        $Create_edit_repairs = $_POST["create_edit_rule"];
    }
    ...
    if($hotel_id != 0){
        if($repair_id == 0){
            if($Create_edit_repairs == 1 || $usert_id == 1){
                $sql="SELECT * FROM `tbl_repair` WHERE `hotel_id` = $hotel_id AND status_id = 6 and is_delete = 0 and saved_status = '$saved_status' ORDER BY 1 DESC";
            }else{
            ...
            $sql="SELECT * FROM `tbl_repair` WHERE  `is_delete` = 0 AND `hotel_id` = $hotel_id AND rpr_id = $repair_id ORDER BY 1 DESC";
        }
        ...
        if ($result && $result->num_rows > 0) {
            while($row = mysqli_fetch_array($result)) {
                ...
                if($is_completed == 0 || $repair_id != 0){
                    $check_one =  1;
                    $temp['rpr_id'] = $row['rpr_id'];
                    $rpr_id= $row['rpr_id'];
                    $temp['title'] = trim($row['title']);
                    $temp['title_it'] = trim($row['title_it']);
                    $temp['title_de'] = trim($row['title_de']);
                    //Hide one
                    if ($repair_id != 0 || $saved_status == 'TEMPLATE' ){
                    ...
                    $sql2="SELECT * FROM `tbl_repair_attachment` WHERE rpr_id = $rpr_id";
                    $result2 = $conn->query($sql2);
                    if ($result2 && $result2->num_rows > 0) {
                        while($row2 = mysqli_fetch_array($result2)) {
                            $temp3['attachment_id'] = $row2['rpra_id'];
                            $temp3['attachment_url'] = $row2['attachment_url'];
                            array_push($temp_attachment, $temp3);
                        }
                    }

                    $sql2="SELECT b.* FROM `tbl_repair_departments` as a INNER JOIN tbl_department as b on a.depart_id = b.depart_id WHERE a.rpr_id = $rpr_id";
                    $result2 = $conn->query($sql2);
                    if ($result2 && $result2->num_rows > 0) {
                        while($row2 = mysqli_fetch_array($result2)) {
                    ...
                    $sql2="SELECT a.*, b.* FROM `tbl_repair_recipents` as a INNER JOIN tbl_user as b on a.user_id = b.user_id WHERE a.rpr_id = $rpr_id";
                    $result2 = $conn->query($sql2);
                    if ($result2 && $result2->num_rows > 0) {
                        while($row2 = mysqli_fetch_array($result2)) {
                    ...
                    $sql2="SELECT * FROM `tbl_repair_recipents` WHERE `rpr_id` = $rpr_id AND `user_id` = $user_id";
                    $result2 = $conn->query($sql2);
                    if ($result2 && $result2->num_rows > 0) {
                        while($row2 = mysqli_fetch_array($result2)) {
