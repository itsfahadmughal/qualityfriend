...
if(file_exists("util_config.php") && is_readable("util_config.php") && include("util_config.php")) 
{
    ...
    if($user_id == 0 || $hotel_id == 0){
        ...
        if($language=='english'){
            $sql="INSERT INTO `tbl_note`(`title`, `description`, `tags`, `saved_status`, `hotel_id`, `is_active`, `is_delete`,`comment`, `entrytime`, `entrybyid`, `entrybyip`, `lastedittime`, `lasteditbyid`, `lasteditbyip`, `visibility_status`) VALUES ('$title','$description','$tag','$save_as','$hotel_id','1','0','$comment','$entry_time','$entryby_id','$entryby_ip','$last_edit_time','$last_editby_id','$last_editby_ip','$visibility_status')";
            ...
            if($result){
                ...
                for ($z = 0; $z < sizeof($attachments_url); $z++) {
                    $sql="INSERT INTO `tbl_note_attachment`( `nte_id`, `attachment_url`, `user_id`) VALUES ('$last_id','$attachments_url[$z]', '$user_id')";
                    $result = $conn->query($sql);
                }
            ...
            if($save_as == "TEMPLATE"){
                ...
                $sql_tour="SELECT * FROM `tbl_note_recipents` WHERE `nte_id` =  $notice_id";
                ...
                $sql_alert_del="UPDATE `tbl_alert` SET `is_delete`= 1 WHERE `id_table_name` = 'tbl_note' AND `hotel_id` = $hotel_id  AND `alert_type` = 'UPDATE' AND `id` = $notice_id";
                $result_alert_del = $conn->query($sql_alert_del);



                $sql="UPDATE `tbl_note` SET `title`='$title',`title_it`='$title_it',`title_de`='$title_de',`description`='$description',`description_it`='$description_it',`description_de`='$description_de',`tags`='$tag',`saved_status`='$save_as',`comment`='$comment',`comment_it`='$comment_it',`comment_de`='$comment_de',`lastedittime`='$last_edit_time',`lasteditbyid`='$entryby_id',`lasteditbyip`='$last_editby_ip',`visibility_status`='$visibility_status' WHERE `nte_id` = $notice_id";

                $sql_d="DELETE FROM `tbl_note_departments` WHERE `nte_id` = $notice_id";
                $result_d = $conn->query($sql_d);

                $sql_d2="DELETE FROM `tbl_note_recipents` WHERE `nte_id` = $notice_id";
                $result_d2 = $conn->query($sql_d2);

                $sql_d3="DELETE FROM `tbl_note_attachment` WHERE `nte_id` = $notice_id";
                ...
                if($result && $result_d2 && $result_d){
                    ...
                    for ($x = 0; $x < sizeof($dep_array_id); $x++) {
                        $sql1="INSERT INTO `tbl_note_departments`(`nte_id`,`depart_id`) VALUES ($notice_id,'$dep_array_id[$x]')";
                        $result1 = $conn->query($sql1);
                        if($save_as == 'CREATE' &&  $visibility_status != "PRIVATE"){
                            $sql_user="SELECT * FROM `tbl_user` WHERE `depart_id` = $dep_array_id[$x]";
                            $result_user = $conn->query($sql_user);
                            if ($result_user && $result_user->num_rows > 0){
                                while($row_user = mysqli_fetch_array($result_user)){
                                    if(!in_array($row_user['user_id'],$recipents_)){
                                        $sql_alert="INSERT INTO `tbl_alert` (`user_id`, `id`, `id_name`, `id_table_name`, `alert_message`, `alert_type`, `hotel_id`, `entrytime`) VALUES ('$row_user[0]','$notice_id','nte_id','tbl_note','$title Updated','UPDATE','$hotel_id','$entry_time')";
                                        $result_alert = $conn->query($sql_alert);
                    ...
                    for ($x = 0; $x < sizeof($recipents_); $x++) {
                        ...
                        $sql1="INSERT INTO `tbl_note_recipents`(`nte_id`,`user_id`,`priority`) VALUES ($notice_id,'$recipents_[$x]',$priority)";
                        $result1 = $conn->query($sql1);
                        ...
                        if($save_as == 'CREATE' &&  $visibility_status != "PRIVATE"){
                            ...
                            $sql_alert="INSERT INTO `tbl_alert`(`user_id`, `id`, `id_name`, `id_table_name`, `alert_message`, `alert_type`, `hotel_id`, `entrytime`,`priority`) VALUES ('$recipents_[$x]','$notice_id','nte_id','tbl_note','$title Updated','UPDATE','$hotel_id','$entry_time',$priority)";
                            $result_alert = $conn->query($sql_alert);
                    ...
                    for ($z = 0; $z < sizeof($attachments_url); $z++) {
                        $sql="INSERT INTO `tbl_note_attachment`( `nte_id`, `attachment_url`, `user_id`) VALUES ('$notice_id','$attachments_url[$z]', '$user_id')";
                        $result = $conn->query($sql);
                    }
